...
def _outage_prob_static_n_links(rate, num_elements):
    if num_elements == 0:
        return np.ones_like(rate)
    else:
        return 1.-np.exp(-(2**rate-1)/num_elements)
def outage_prob_static_phases_approx(rate, num_elements, conn_prob):
    _binom = stats.binom(n=num_elements, p=conn_prob)
    _weights = [_outage_prob_static_n_links(rate, _n)*_binom.pmf(_n)
                for _n in range(num_elements+1)]
    return np.sum(_weights, axis=0)
...
def main(num_elements, connect_prob=[1.], plot=False, export=False):
    ...
    for _num_elements, _conn_prob in it.product(num_elements, connect_prob):
        ...
        out_static = outage_prob_static_phases_approx(rate, _num_elements, _conn_prob)
